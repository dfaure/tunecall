...
4. Build: `cargo run -- --pdf <book>.pdf --offset N --title "<title>"` (per-book offsets and titles live in `indexer/rebuild_db.sh`; a clamp warning usually means the offset is wrong). The `--title` is the human-readable book name shown in the viewer's Books list — read it off the cover (scan page 1) the same way you transcribe the TOC; it's stored in a `meta(key,value)` table in the `.db`. Add the book's `index <stem> <offset> "<title>"` line to `rebuild_db.sh` (the `index()` helper requires all three).
